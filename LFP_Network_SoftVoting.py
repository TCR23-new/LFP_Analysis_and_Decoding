...
from preproc_methods import HighMidLow_Encoding,CollectBands_AmpPhase
from training_methods import Trainer_Ensemble
from neural_nets import Spk_LFP_Net2
...
if __name__ == '__main__':
    ...
    #
    ampdata_spks = np.concatenate((ampdata,spks,phdata),axis=1)
    ...
    for i, (train_index, test_index) in enumerate(rkf.split(np.array(range(ampdata_spks.shape[0])).reshape(-1,1),val_data)):
      ...
      #
      model0 = Spk_LFP_Net2(numBands,train_X.shape[2],100,3)
      scalers,model,training_Data = Trainer_Ensemble(ampdata_spks,model0,val_data,training_Data,device,lr)
    ...
    # save stuff
    pickle.dump([split_testMetrics],open('{}LFP_ModelType_NewArch_Training_job_{}_vflag_{}_wSpks_SoftVotingPhase.pickle'.format(savedir,jobid,valflag),'wb'))
